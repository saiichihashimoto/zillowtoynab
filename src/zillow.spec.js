...
describe('zillow', () => {
	...
	describe('getZestimate', () => {
		it('gets the zestimate', () => expect(getZestimate('ZPID')).resolves.toEqual(expect.objectContaining({ amount: 1000, date: '2018-01-02' })));

		it('uses the correct options', async () => {
			await getZestimate('ZPID');
		...
		it('rejects with xml code', () => {
			...
			return expect(getZestimate('ZPID')).rejects.toHaveProperty('statusCode', 1);
		});
	});
});
